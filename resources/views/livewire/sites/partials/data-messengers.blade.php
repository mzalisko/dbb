                ...
                <div wire:click="editEntry({{ $msg->id }})" class="failover-row failover-row--primary">
                    ...
                    <span class="failover-actions">
                        ...
                        @elseif($msg->backups->count() > 0)
                            <button class="cc-trigger" wire:click.stop="triggerFailover({{ $msg->id }})" wire:confirm="Перемкнути на резерв? Працюватиме наступний у черзі." title="Перемкнути на резерв" aria-label="Перемкнути на резерв">
