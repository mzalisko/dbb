            ...
            @forelse ($phonePrimaries as $i => $phone)
                <div style="border-top:{{ $i ? '1px solid var(--ink-3)' : 'none' }};">
                    ...
                    <div wire:click="openPhone({{ $phone->id }})"
                         style="display:grid; grid-template-columns:32px 1.4fr 1fr 100px 80px; gap:12px; padding:14px 18px; align-items:center; cursor:pointer;"
    ...
    @if ($openPhoneId)
        ...
        @if ($ph)
            <x-ui.drawer
                :open="true"
                :title="$ph->value"
                :sub="$ph->label . ' · ' . $ph->geo_label"
                @drawer-close.window="$wire.closePhone()"
            >
                <div style="display:flex; flex-direction:column; gap:20px;">
                    <div>
                        <label class="label">Номер</label>
                        <div class="mono" style="font:20px var(--font-mono); color:var(--ink-9); margin-top:8px;">{{ $ph->value }}</div>
                    </div>
                    <div>
                        <label class="label">Мітка</label>
                        <input class="input" value="{{ $ph->label }}" readonly />
                    </div>
                    <div>
                        ...
                        <div style="display:flex; gap:8px; margin-top:8px;">
                            @foreach ([['primary', 'Головний'], ['backup', 'Резерв'], ['archive', 'Архів']] as [$k, $l])
                                <span style="flex:1; padding:10px; text-align:center; border-radius:4px; font:13px var(--font-sans);
                                    ...
                                    color:{{ $ph->role === $k ? 'var(--paper)' : 'var(--ink-7)' }};">
                                    {{ $l }}
                                </span>
                            @endforeach
                        </div>
                    ...
                    <div>
                        ...
                        <div style="margin-top:8px; font:13.5px var(--font-sans); color:var(--ink-7);">{{ $ph->geo_label ?: 'Усі гео' }}</div>
                    </div>
                </div>
